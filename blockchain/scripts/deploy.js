...
async function main() {
  ...
  try {
    ...
    // Copy to frontend
    const frontendPath = path.join(__dirname, '..', '..', 'client', 'lib');
    if (!fs.existsSync(frontendPath)) fs.mkdirSync(frontendPath, { recursive: true });
    fs.writeFileSync(path.join(frontendPath, 'contract-info.json'), JSON.stringify({
      address: contractAddress,
      chainId: (await ethers.provider.getNetwork()).chainId.toString(),
      abi: ITAssetManager.interface.formatJson()
    }, null, 2));
    
    console.log("📋 Contract info copied to frontend");
